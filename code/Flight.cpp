...
void * Flight::playSeconds(){
	if(this->seconds == 30) speaker->play(std::to_string(this->seconds));
...
void * Flight::start()
{	
	//~ if (minuts < 15 && >=10) numberOfRaces = 2; else if(minuts < 10) numberOfRaces = 1;
	puts("Flight lancé\n");
	isActive = true;
	char car;
	...
	while (this->isActive)
	{
		while((this->minuts >= 0) && (!this->stopNow))
		{
			...
			{
				if((this->minuts == 15 || this->minuts == 10 || this->minuts == 5) && this->seconds < 30) {	
